...
@Component({
  selector: 'app-infinite-menu',
  standalone: true,
  imports: [CommonModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './infinite-menu.component.html',
  styleUrls: ['./infinite-menu.component.scss'],
})
export class InfiniteMenuComponent implements AfterViewInit, OnChanges, OnDestroy {
  /** Array of menu items to display on the sphere. */
  ...
  // ── Private state ─────────────────────────────────────────────────────────
  private sketch: InfiniteGridMenu | null = null;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Re-initialise when @Input() properties change after the first render.
    if ((changes['items'] || changes['scale']) && this.canvasRef) {
      this.destroySketch();
      this.initSketch();
    }
  }
  ...
  private initSketch(): void {
    ...
    const effectiveItems = this.items?.length ? this.items : defaultItems;

    // Callback fires from the WebGL render loop (outside Angular zone).
    // We must re-enter the zone so that change detection picks up state changes.
    const handleActiveItem = (index: number): void => {
    ...
    const handleMovementChange = (moving: boolean): void => {
      this.ngZone.run(() => {
    ...
    // Run the WebGL render loop outside Angular zone to prevent
    // requestAnimationFrame from triggering change detection every frame.
    ...
    this.resizeHandler = (): void => this.sketch?.resize();
